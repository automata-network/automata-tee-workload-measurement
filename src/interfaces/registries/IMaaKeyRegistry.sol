...
///
///      Admin operations are gated by the contract's EVM owner (OwnableUpgradeable). The
///      MAA-registry trust anchor is global state shared by every Azure CVM that registers
///      against this SessionRegistry deployment, so it warrants the simplest gate; the
///      owner key should be a multisig or other high-assurance custody. A compromised owner
///      can register arbitrary MAA signing keys and break the Azure trust chain end-to-end.
