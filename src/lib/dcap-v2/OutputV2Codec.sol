...
import {OutputV2} from "./OutputV2.sol";
...
library OutputV2Codec {
    ...
    function decode(bytes memory data) internal pure returns (OutputV2 memory out) {
        check(data.length >= HEAD_LENGTH && data.length <= MAX_LENGTH);
        out.formatMajorVersion = uint16(LibBytes.readBytes2(data, 0));
        out.formatMinorVersion = uint16(LibBytes.readBytes2(data, 2));
        check(out.formatMajorVersion == 2 && out.formatMinorVersion == 1 && data[4] == 0x06);
        out.quoteVersion = uint16(LibBytes.readBytes2(data, 5));
        out.quoteBodyType = uint16(LibBytes.readBytes2(data, 7));
        out.tcbStatus = uint8(data[9]);
        check(out.tcbStatus <= 9);
        out.fmspc = bytes6(LibBytes.readBytes32(data, 10));
        out.ppid = bytes16(LibBytes.readBytes32(data, 16));
        out.piid = bytes16(LibBytes.readBytes32(data, 32));
        check(uint8(data[48]) <= 1);
        out.piidPresent = data[48] == 0x01;
        check(out.piidPresent || out.piid == bytes16(0));
        bodyLength(out.quoteVersion, out.quoteBodyType);
        uint256 advisoryOffset = uint16(LibBytes.readBytes2(data, 49));
        uint256 advisoryLength = uint16(LibBytes.readBytes2(data, 51));
        if (advisoryLength == 0) {
            ...
            bytes memory advisory = LibBytes.slice(data, advisoryOffset, advisoryLength);
        ...
        out.timestamp = uint64(bytes8(LibBytes.readBytes32(data, 53)));
        for (uint256 i; i < 6; ++i) {
            out.collateralHashes[i] = LibBytes.readBytes32(data, 61 + i * 32);
        }
        out.fullQuoteHash = LibBytes.readBytes32(data, 253);
        out.quoteBodyHash = LibBytes.readBytes32(data, 285);
    }
