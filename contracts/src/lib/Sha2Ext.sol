...
library Sha2Ext {
    ...
    function sha384(bytes memory message) internal pure returns (Bytes48 memory) {
        uint64[8] memory h = [
            0xcbbb9d5dc1059ed8,
            0x629a292a367cd507,
            0x9159015a3070dd17,
    ...
    function padMessage(bytes memory message) internal pure returns (bytes memory) {
        ...
        bytes memory tail = message.substring(messageLength - mdi, messageLength);
    ...
    function getBlock(bytes memory message, uint64[16] memory blocks, uint256 index) internal pure {
        for (uint256 i = 0; i < 16; ++i) {
            blocks[i] = uint64(message.readBytes8(index + i * 8));
        }
    }
