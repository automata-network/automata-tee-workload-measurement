...
contract DeployTeeVerifier is DeploymentConfig {
    function _deployTeeVerifier() internal returns (address) {
        address dcapAttestationAddr = vm.envAddress("DCAP_V2_ATTESTATION_ADDR");
        require(dcapAttestationAddr.code.length != 0, "DCAP V2 has no code");
        IDcapAttestationV2(dcapAttestationAddr).programModeV2(IDcapAttestationV2.ZkCoProcessorType.Succinct, bytes32(0));
        ...
        TeeVerifier teeVerifier = new TeeVerifier{salt: TEE_VERIFIER_SALT}(
            IDcapAttestationV2(dcapAttestationAddr), IZkVerifierRegistry(zkVerifierRegistryAddr)
        );
